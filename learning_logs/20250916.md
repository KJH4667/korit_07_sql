...
3. orders에서 월별로 주문한 회원수를 출력할 것(주문일자(order_data) 컬럼을 활용하고, 최신 순으로 정렬할 것).
```sql
...
```
  ...
  3. users에서 이하의 국가(country)에 거주 중인 회원(id)수를 국가별로 추출할것(단, 회원 수가 5명 이상인 국가만 추출하고, 회원 수 기준으로 내림 차순 정렬 할 것)
